...
impl Conductor {
    ...
    pub fn apply(&mut self, command: Command) {
        ...
        match cmd {
            ...
            CmdArg::Start => self.start(args),
            CmdArg::Pause => self.pause(args),
            CmdArg::Resume => self.resume(args),
            CmdArg::Stop => self.stop(args),
    ...
    fn pause(&mut self, args: String) {
        ...
        let first = match args.next() {
            Some(string) => string,
            None => {
                println!("\nErr: Not enough arguments for pause");
                return;
            }
        };
    ...
    fn resume(&mut self, args: String) {
        ...
        let first = match args.next() {
            Some(string) => string,
            None => {
                println!("\nErr: Not enough arguments for resume");
                return;
            }
        };
    ...
    fn stop(&mut self, args: String) {
        ...
        let first = match args.next() {
            Some(string) => string,
            None => {
                println!("\nErr: Not enough arguments for stop");
                return;
            }
        };
        ...
        let mut t_arg = |t: &str, current: Rc<RefCell<TempoState>>| -> Result<Rc<RefCell<TempoState>>, &'static str> {
            ...
            if u_str == "c:" {
                ...
                match self.tempo_cons.get(&name.to_string()) {
                    Some(tc) => return Ok(Rc::clone(tc)),
                    None => return Err("\nErr: Couldn't find TempoContext"),
                }
            } else {
        ...
        while let Some(arg) = args.next() {
            match arg {
                "-t" | "--tempo" => {
                    match args.next() {
                        Some(t) => {
                            tempo_state = match t_arg(t, tempo_state) {
                                Ok(state) => state,
